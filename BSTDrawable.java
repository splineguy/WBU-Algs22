/******************************************************************************
 *  Compilation:  javac-algs4 BSTDrawable.java
 *  Execution:    java-algs4 BSTDrawable
...
public class BSTDrawable<Key extends Comparable<Key>, Value> extends BST<Key, Value> {
    ...
    public void drawSimple() {
        treeLevel = 0;
        setCoordinates(root, 0.9, 0.05);

        StdDraw.setPenColor(StdDraw.BLACK);
        drawLines(root);
        drawSimpleNodes(root);
    }

    private void setCoordinates(Node node, double distance, double dy) {
        if (node == null) {
            return;
        }

        setCoordinates(node.left, distance - dy, dy);
        node.xCoordinate = (0.5 + treeLevel++) / size();
        node.yCoordinate = distance - dy;
        setCoordinates(node.right, distance - dy, dy);
    }
    ...
    private void drawSimpleNodes(Node node) {
        if (node == null) {
            return;
        }
        ...
        drawSimpleNodes(node.left);

        StdDraw.setPenColor(StdDraw.BLACK);
        // Clear the node circle area
        StdDraw.filledCircle(node.xCoordinate, node.yCoordinate, nodeRadius);

        drawSimpleNodes(node.right);
    }

    public static void main(String[] args) {


        BSTDrawable<String, Integer> st = new BSTDrawable<String, Integer>();
